...
router.get("/get-my-matches", authorization, async (req, res) => {
  try {
    ...
    const dbRes = await pool.query(
      `select ga.game_id,us1.user_email as player1_email,us2.user_email as player2_email,ga.game_type,win.user_email as winner_email , ga.in_tournament
      from "games" ga
...
router.get("/get-tournaments-matches", authorization, async (req, res) => {
  try {
    ...
    //1.request to the database
    const dbRes = await pool.query(
      `select  ga.game_id,us1.user_email as player1_email,us2.user_email as player2_email,win.user_email as winner_email 
      ,ga.game_type, tourn.tournament_id,tourn.tournament_name,tga.phases,tga.phase,tga.phase_id,
      tourn.finished as tournament_finished,ga.in_tournament,tga.endgame,
      tga.phases,tga.phase_id,tga.phase
